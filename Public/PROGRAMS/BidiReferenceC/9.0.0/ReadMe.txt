...
Version 9.0.0 (for UBA Version 9.0.0)

Code Release Date: October 6, 2016 

Documentation Update: October 6, 2016
...
for UBA 6.3.0. Unicode 9.0 support uses the same rule set as for
UBA 8.0.0, but updates the repertoire to Unicode 9.0.
...
for all versions from UBA 6.2.0 through UBA 9.0.0.
...
-unn The -u flag allows choice of which UBA version to run (and which
     version data files to read). Valid values are -u62, -u63, -u70, -u80, and -u90.
     bidiref defaults to UBA 9.0.0 rules.
...
Parse BidiRefTest.txt and run the UBA 9.0.0 rules on each
...
Parse BidiTest.txt and run the UBA 9.0.0 rules on each
...
BidiCharacterTest.txt, run the UBA 9.0.0 rules on each
test case, printing out the results with the most verbose
level of debug output.

% bidiref -d4 -p c:\test\90test\ MyOwnTestCases.txt

Parse MyOwnTestCases.txt, using the format specified for
BidiCharacterTest.txt, run the UBA 9.0.0 rules on each
test case, printing out the results with the most verbose
level of debug output. Bidiref will look for
UnicodeData.txt and BidiBrackets.txt in the c:\test\90test\
...
can do so. The trace flags supported (as of Version 9.0) are:
...
Unicode 9.0, is 130 characters, but the longer input string
...
If no datapath is specified, bidiref requires the following data files
...
will look for the following UCD data files in the datapath directory:

For *any* version of UBA:
...
This handling of a test case input file only applies to the
full bidiref command line utility, and not to the simple API
for the bidi reference code documented in bidiref.h and below.
...
provided, the character property tables will be sought in
that specific location. Otherwise, they require specific
file names in the current directory (see above).
...
Note: As of Version 9.0, this API is not multithread safe,
