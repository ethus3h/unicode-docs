...
Version 7.0.0 (for UBA Version 7.0.0)

Code Release Date: June 4, 2015 

Documentation Update: June 4, 2015
...
-unn The -u flag allows choice of which UBA version to run (and which
     version data files to read). Valid values are -u62, -u63, and -u70.
     bidiref defaults to UBA 6.2.0 rules.
...
Parse BidiRefTest.txt and run the UBA 6.3.0 rules on each
test case, printing out results plus minimal debug output,
using Unicode 7.0 UCD data files, instead of Unicode 6.3.
...
can do so. The trace flags supported (as of Version 7.0) are:
...
Unicode 7.0, is 76 characters, but the longer input string
...
Note: As of Version 7.0, this API is not multithread safe,
