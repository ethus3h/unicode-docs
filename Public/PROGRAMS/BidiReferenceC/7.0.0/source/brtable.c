...
static int br_parsePropertyData ( int status, int version )
{
FILE *fdi;
int rc;
int i;
...
int rc;
	...
	rc = br_parsePropertyData ( UNICODE_DATA, version );
    if ( rc < 0 )
    {
        if ( Trace ( Trace15 ) )
        {
    ...
    if ( version > UBA62 )
    {
        rc = br_parsePropertyData ( BRACKET_DATA, version );
        if ( rc < 0 )
        {
            if ( Trace ( Trace15 ) )
            {
