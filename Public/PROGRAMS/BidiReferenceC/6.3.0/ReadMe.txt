...
Version 1.8 (for UBA Version 6.3.0)

Code Release Date: July 8, 2013 

Documentation Update: October 1, 2013

Author: Ken Whistler

The UBA Reference Implementation is furnished in three forms:

1. A generic Windows executable (bidiref.exe).

2. Platform-independent source code, which can be compiled for an
   executable to run on almost any modern platform.

3. A compiled Windows 32-bit library (libbidir.lib) and a short public 
...
The bidiref implementation supports both UBA 6.2.0 and UBA 6.3.0 rules, 
and an effort has been made to keep the expression of both versions
as parallel as possible, so as to highlight the explicit differences
introduced in UBA 6.3.0. Where the logic of the rule application does
...
in the code for both versions. Where rules are distinct in their
logic, the rule tracing makes it obvious where UBA 6.3.0-specific
...
The reference algorithm supports both a fully conformant UBA 6.2.0
implementation and a fully conformant UBA 6.3.0 implementation.
...
-x  bidiref defaults to UBA 6.2.0 rules. The -x flag sets the program to
    follow UBA 6.3.0 rules, instead.
...
If the -z flag is not specified, bidiref defaults to an input file name
...
either UBA 6.2.0 rules or UBA 6.3.0 rules against exactly the
...
Parse BidiRefTest.txt and run the UBA 6.2.0 rules on each
...
Parse BidiTest.txt and run the UBA 6.2.0 rules on each
test case, printing out a summary of results only.

% bidiref -x BidiTest.txt
...
% bidiref -x -d2 BidiTestSmallSet2.txt
...
% bidiref -x -d3 BidiCharacterTest.txt
...
BidiCharacterTest.txt, run the UBA 6.2.0 rules on each
...
can do so. The trace flags supported (as of Version 1.8) are:
...
The longest test case input string currently supported is 76
characters long. (This corresponds to the longest current
test case defined in BidiTest.txt for UBA 6.3.0.)
...
support UBA 6.3.0 rules and the BidiCharacterTest.txt format
...
[This requirement for exact file names in the current directory may
...
(UnicodeData-6.2.0.txt when invoked for UBA 6.2.0 rules, and
UnicodeData-6.3.0.txt and BidiBrackets-6.3.0.txt when invoked
for UBA 6.3.0 rules), to build up the internal tables
...
A compiled 32-bit version of the static library is provided for the
Windows platform: libbidir.lib. [TBD: add compiled version
for linuxamd64, Win64 and/or other platforms.]
...
Note: As of Version 1.8, this API is not multithread safe,
